...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (event) => {
        if (!hiddenBlock.contains(event.target) && !showBlockButton.contains(event.target) && !showBlockButtonPhone.contains(event.target)) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function displaySelectedProducts(cart, total_price) {
        if (selectedProductsSection) {
            ...
            if (Object.keys(cart).length === 0) {
                selectedProductsSection.innerHTML = '<p>Your cart is empty.</p>';
            } else {
    ...
    document.querySelectorAll('.addToCartBtn').forEach(button => {
        button.addEventListener('click', function () {
            ...
            if (productDetail) {
                ...
                const availableColors = productDetail.querySelectorAll('.detail-color');
                if (availableColors.length > 0) {
                    const selectedColor = productDetail.querySelector('.detail-color.selected');
                    if (selectedColor) {
    ...
    document.querySelectorAll('.detail-color').forEach(color => {
        color.addEventListener('click', function () {
            color.parentNode.querySelectorAll('.detail-color').forEach(otherColor => {
